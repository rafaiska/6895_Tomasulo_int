...
//VARIAVEIS DE CLOCK
uint8_t clock_state = 0; //Valor 0 = baixa tensao, valor 1 = alta tensao
uint32_t clock_count = 0; //Conta pulsos de clock desde o inicio
...
uint8_t tomasulo_exit = 0;			//Indica fim da simulacao
uint8_t n_uf_soma = 4;				//Numero de unidades funcionais de soma
uint8_t n_uf_mult = 2;				//Numero de unidades funcionais de multiplicacao
uint8_t n_uf_divi = 2;				//Numero de unidades funcionais de divisao
uint8_t n_buff_load = 2;			//Numero de buffers de load
uint8_t n_buff_store = 2;			//Numero de buffers de store
uint8_t n_busca_ciclo = 4;			//Numero de instrucoes buscadas e decodificadas em um ciclo de clock
